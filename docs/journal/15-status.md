...
### 9.0.70 批次 A-γ1 —— 核心状态序列:混乱目标重定向全链(2026-09-24)

2026-09-24 交付。补全核心状态 1..11 中混乱(`BATTLE_ST_CONFUSION`)的行动重定向机制。

### 1. 源码事实与裁定

1. **触发时机与概率**(SSRC80 `battle.c:5646-5648`):
   处于混乱状态的角色轮到行动时,掷骰 `rand(1, 100) > 80` 时维持原指令(20% 概率);`<= 80` 时触发混乱重定向(80% 概率)。
2. **指令与目标改写**(`:5649-5664`):
   - 指令强制改写为普通攻击 `ATTACK`;
   - 目标选择随机化:`side = rand(0, 1); pos = rand(0, 9);`
   - 从 `(pos + 1) % 10` 开始顺时针循环 10 个槽位探测:
     - 排除自身(`def_no == actor_slot`);
     - `targetCheck(field, dead, def_no)` 存活且可用者即作为目标;
   - 全场无除自己外的可用目标时,选定 -1(本回合不行动)。
3. **架构落地形态**:
   - 纯函数 `rollConfusionRedirect` 放入 `shared/rules/Battle.{h,cpp}`;
   - `resolveTurn` 在 `tick_one` 之后、指令分发之前拦截混乱角色的指令,实现对逃跑/防御/技能/道具的统合改写。
