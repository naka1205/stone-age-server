...
| 2026-09-07 | ★ **L2 首两个实体族落地(结构层,批次 M.1)**:`shared/model/Pet.h` + `Player.h`。★ 字段清单**不是自拟的**,取自原版 `PET_createPetFromCharaIndex`(展开视图 `char/pet.c:325-405`)实际拷贝的 30 项,建 / 不建逐条记明。已兑现本文四条:**M1/§4.2** —— Player 与 Pet 是两个独立类型,三对同槽异义(`MODAI`=`CHARM` · `VARIABLEAI`=`LUCK` · `PETGETLV`=`CHATVOLUME`)在注释里逐个点名;**M2** —— `kKind` 让类型与族在编译期绑定;**M6** —— `CHAR_ALLOCPOINT` 拆成 4 个 `uint8_t` 成长率,⚠️ 三个夹取上限归属**不同路径**(0..255 由类型兑现 / ≤60 进化孵化 / ≤50 死亡扣属),★ **捕获路径源码一处夹取都没有 ⇒ 本批不加**(加一个源码没有的夹取就是引入原版没有的行为);**§8.2** —— 会话 → Player 走 `ConnIndex` 哈希索引。⚠️ **§4 的 65 条别名展开、§11 欠债 1(693 字段逐条含义)与欠债 2(隐式复用)仍未动**。<br>⚠️★ 另记一处**本文未预见的缺口**:捕获时**敌人侧没有 L2 实体** —— `Rules::Combatant` 是战斗输入子集(存的是已重算的 attack/defense/quick),**没有 vital/str/tough/dex 也没有名字**,而源码是从 `enemyindex`(完整 Char 实体)拷 ⇒ 捕获出的宠物那四项是 **0**,补齐要等 `kEnemy` 族 + L4 内容导入(`00` §9.0.26 ③)。<br>⚠️ Y 五项初值(`YHP`/`YATK`/`YDEF`/`YQUICK`/`YLV`)**有意不建** —— 它们是 `CHAR_complianceParameter` 的产物,而属性推导未移植;建了只能填 0,而 0 会被后续成长逻辑读成「初始值就是 0」,**比字段不存在更危险** |
| 2026-09-09 | ★★ **L2 第三个实体族落地(结构层,批次 M.4b)**:`shared/model/Enemy.h`。★ 字段清单**不是自拟的**,取自原版 `ENEMY_createEnemy`(展开视图 `char/enemy.c:994-1180`)实际写入 `CharNew` 的那些槽,建 / 不建**逐条记明**(文末 11 条),同 `Pet.h` 的纪律。已兑现本文四条:**§2.1** —— Enemy 的族特征「由模板生成、无存档」两半都有结构后果(四维是 `rollSpawnStats` 当场摇的、没有 cdkey、**敌人侧不建 Y 五项**);**M2** —— `kKind` 编译期绑定,`model_pool` 断言三族两两不同类型;**M6** —— `CHAR_ALLOCPOINT` 同样拆成 4 个 `uint8_t`,且**创建路径一处夹取都没有**(同 `Pet`);**§8.1 / M10** —— 池 + generation 句柄,悬空即 nullptr。<br>⚠️★★ **本批为 §4 的 65 条别名展开新增了一个"最干净的样本",值得单独记**:`CHAR_VARIABLEAI == CHAR_LUCK`(§4.2 的 19 条之一,`char_base.h:637`)—— `enemy.c:1076` 以「AI 变量」的名义往那个槽写 **0**,而 `pet.c:347` 以「幸运」的名义把它读出来拷进宠物 ⇒ **捕获出的宠物幸运在原版里就恒 0**,并且**敌人从头到尾没有"幸运"这个属性**。★ 于是 `Model::Enemy` **不建 `luck`**、`Model::Pet` 两个字段都建,捕获处写 `pet->luck = src_enemy->variable_ai` 而不是 `= 0`(让别名**在代码里看得见**)。★★ **它之所以是最干净的样本**:两个名字的值**恰好都是 0** ⇒ 若把两族合成宽表、把两个别名并成一个字段,**行为完全不变、连测试都抓不到** —— 这正是 **M1**「合并任意两个别名都是静默错误」要防的东西,而通常的样本(门的关门时刻 vs 商店客户 index)靠数值明显不同就能发现。⇒ `model_pool` 用 SFINAE 探针把两族字段面的差别钉成**编译期断言**(`Pet` 有 `luck` / `Enemy` 没有),给 Enemy 加个 `luck` 会当场转红。<br>★ 另:**`Pet` 的 Y 五项本批建起来了**(M.1 变更记录里「有意不建」的理由随四维有源而解除),⚠️★ 并纠正了当时的一句转述 —— 那条写着 Y 五项会被「后续成长逻辑读成初始值」,而**源码里没有任何计算读它们**(写点 4 处;读点只有存档键表 + 两处下发显示串,中文名「初体力/初攻击/…」印证)⇒ 它属**显示 + 存档**面。⚠️ **§4 的 65 条别名展开、§11 欠债 1(693 字段逐条含义)与欠债 2(隐式复用)仍未动。** |
| 2026-09-10 | ★★ **第一个"不进五族"的池落地:Item 族 + 背包槽(结构层,批次 I.1,道具域第一批)**:`shared/model/Item.h` + `Player.h` 加 `items[54]`。★★ **本批给 §2.1 五族划分补了一个反例的正解**:背包道具**不是** `Char` 实体族 —— 五族判别键是 `CHAR_TYPE`,而背包道具在原版是**独立全局池** `ITEM_item[itemnum]`(`item.c:486`)成员,角色经 `CHAR_getItemIndex` 存池下标引用它(`item.c:497`),**不在** `CHAR_chara[]` 三段式角色池。⇒ Item 建**独立 `EntityPool<Item,10000>` + `ItemHandle`**(= `EntityHandle` 的语义别名,机制复用),**不塞进 `EntityKind`**(塞进去会污染「实体族 = CHAR_TYPE」这条 M2 硬约束)。⚠️ §2.1 表里 `WorldObject`「地面道具不在 Char 池里」说的是**地面**道具;背包道具与地面道具是否共用本 `Item` 结构、共用 10000 池,留掉落批次定。字段清单**不是自拟的**,取展开视图 `ITEM_DATAINT`/`ITEM_DATACHAR` 启用下标 + `itemset6.txt` 实列,只建背包地基必需(id/name/unique_code/type/level/cost/堆叠两列/掉落两列/主人反指),文末逐条登记不建(装备加成/合成/镶嵌/魔法/状态附加各属其域;★★ **functable/Lua 全段不复刻** —— 8.0 无 Lua,同 §2.3「不复刻字符串→函数指针运行期绑定」+ `04 §3.3.3`)。⚠️ 兑现本文:**M2** `Item` 独立类型(但无 `kKind` —— 它不是族,`model_pool` 断言 `kEntityKindCount` 仍是 5)· **M10** 主人反指 `owner` 带 generation、悬空即 nullptr(同 `Pet.owner`)· **POD/M6** `Item` 平凡可拷。⚠️ **本批不接扣/掉/用任一写入链路**(观察面 `itemCount`/`playerItemSlotsUsed` 恒 0,用例断言之);§4 的 65 条别名展开、§11 欠债 1/2 仍未动。 |
