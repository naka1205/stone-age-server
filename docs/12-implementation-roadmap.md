...
| 09-24 | **批次 A-γ1**（§9.0.70：核心状态序列混乱重定向 `rollConfusionRedirect` 落地，80% 触发普攻重定向至全场存活目标；`rules_battle` 147→149 例/3,035→3,048 断言，RV-1 反向验证通过）＋ **CI 工具 macOS SDK 自动适配** | 批次 A-δ 宠技战斗侧（`S_PetOut` 强制换宠等）或启动 NPC 基础框架切片 |
