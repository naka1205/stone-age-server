...
| P2 最小可玩流程 | 登录选角 → 一张真实地图 → 移动遇敌 → 手动战斗/捕获 → 奖励 → 退出重登仍保留；存储与真实内容导入沿用既有架构 | 本地完整流程、重启和真实存储验证通过；[发布验收中](audits/2026-09-12-playable-loop.md) |
| P3 所需资源 | F7 调色板 → F5 覆盖关系/F6 图集分组；只转换 P2 的地图、玩家和敌人所需资产 | 3,910 图元/9 图集已接入；上下方向、地图投影和战斗站位已修正并截图验证，随 P2 发布 |
