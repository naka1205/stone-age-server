...
enum class LogEvent : std::uint16_t
{
	...
	kBattleJoined = 207,
	// ★ 捕获判定通过、但世界写提交不了(2026-09-07,批次 M.1 L2 接线时补)。
	//
	// ⚠️★ **它记的是一处已知的不对称,不是一个普通错误**:`CaptureAct` 事件在 L3 判定
	//    通过时就已发给客户端(flags != 0),而"生成宠物"这一步可能在世界写阶段才失败
	//    (主人宠物槽满 / 宠物池满)—— 源码里这两件事在同一个函数里,`flg` 能被改回 0;
	//    我们分成 L3 判定与调用方写回两段,**事件发出去就改不回来了**。
	//    ⇒ 客户端会演"抓到了"而服务端没给宠物。这个不一致要在日志里有名字,
	//      否则它就是 00 §10.4 那类"不报错、数据不对"的静默错误。
	// ★ 级别 error 不是 warn:与 kBattleJoinFailed 同一理由 —— 玩家侧的表现是
	//   「明明抓到了却没有宠物」,而那是客服会收到的投诉,不是可以淹在 warn 里的东西。
	kCaptureCommitFailed = 208,
	// ★ L2 实体池耗尽(2026-09-07,批次 M.1)。
	//
	// ⚠️★ 容量是**硬上限**:`EntityPool<T, N>` 的 N 是模板参数,`allocate()` 满时返回
	//    空句柄而**不会悄悄扩容** —— 那正是 15 §9.1 三根支柱「运行期零分配」要的性质,
	//    代价就是每个调用点都必须判、判失败必须报。
