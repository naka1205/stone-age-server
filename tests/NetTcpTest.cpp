...
class RecordingEvents final : public TransportEvents
{
  public:
	...
	void onDisconnected(ConnectionId id) { disconnected.push_back(id); }
...
TEST_CASE("端口 0 由系统分配,并能回读出来")
{
	...
	CHECK(t.listenPort() != 0);
	CHECK(t.connectionCount() == 0u);
}
...
TEST_CASE("重复 Listen 是调用方的逻辑错,不是可恢复状态")
{
	...
	CHECK(std::strlen(t.lastError()) > 0);
}
...
TEST_CASE("接受连接 ⇒ OnConnected;对端关闭 ⇒ OnDisconnected")
{
	...
	{
		TestClient c;
		REQUIRE(c.connect(t.listenPort()));
		REQUIRE(pumpUntil(t, [&]
		                  { return !ev.connected.empty(); }));
		CHECK(t.connectionCount() == 1u);
		c.close();
	}

	REQUIRE(pumpUntil(t, [&]
	                  { return !ev.disconnected.empty(); }));
...
TEST_CASE("多条连接各自独立编号")
{
	...
	TestClient a, b;
	REQUIRE(a.connect(t.listenPort()));
	REQUIRE(b.connect(t.listenPort()));
	REQUIRE(pumpUntil(t, [&]
	                  { return ev.connected.size() == 2; }));
	CHECK(ev.connected[0] != ev.connected[1]);
	CHECK(t.connectionCount() == 2u);
}
...
TEST_CASE("上行字节原样交给上层")
{
	...
	REQUIRE(pumpUntil(t, [&]
	                  { return ev.bytes.size() >= payload.size(); }));
...
TEST_CASE("下行:Send 的字节客户端收得到")
{
	...
	REQUIRE(pumpUntil(t, [&]
	                  { return t.pendingOutbound(ev.connected[0]) == 0; }));
...
TEST_CASE("★★ 大负载往返 —— 内核缓冲装不下时必须排队续写,不能截断")
{
	...
	REQUIRE(c.connect(t.listenPort(), 64 * 1024));
	REQUIRE(pumpUntil(t, [&]
	                  { return !ev.connected.empty(); }));
	...
	while (big.size() < kCap)
	{
		...
		std::size_t stable = 0;
		std::size_t last_pending = t.pendingOutbound(conn);
		pumpUntil(
		    t,
		    [&]
		    {
			    const std::size_t now = t.pendingOutbound(conn);
			    stable = (now == last_pending) ? stable + 1 : 0;
			    last_pending = now;
			    return stable >= 50 || now == 0;
		    },
		    5000);
		backlog = t.pendingOutbound(conn);
		if (backlog > 0)
			break;
	}
	...
	std::thread reader([&]
	                   { received = c.recvExactly(big.size(), got); });

	const bool drained = pumpUntil(
	    t, [&]
	    { return t.pendingOutbound(conn) == 0; }, 10000);
	reader.join();
  ...
  public:
	             ...
	             std::size_t n) override
	{
		...
		for (;;)
		{
			...
			std::uint32_t len = 0;
			if (_reader.next(&p, &len) != FrameStatus::kOk)
				break;
			std::vector<std::uint8_t> out;
			const bool ok = _session->handleFrame(p, len, out);
			_reader.pop();
			if (!out.empty())
				_t.send(_conn, out.data(), out.size());
			if (!ok)
			{
...
TEST_CASE("★★ 端到端:真 socket 上握手成功并收到 HandshakeAccepted")
{
	...
	REQUIRE(pumpUntil(t, [&]
	                  { return !bridge.ready.empty() && t.pendingOutbound(bridge.conn()) == 0; }));
